...
function csurf (options) {
  ...
  return function csrf (req, res, next) {
    ...
    req.csrfToken = function csrfToken () {
        console.log(">>>> ##3 csrfToken first check:: ",{cookie, secret, gs: getSecret(req, sessionKey, cookie)})
      var sec = !cookie
        ? getSecret(req, sessionKey, cookie)
        : secret

      // use cached token if secret has not changed
      console.log(">>>> #### ",{token, sec, secret} )
      if (token && sec === secret) {
        return token
      }
      ...
      if (sec === undefined) {
        ...
        console.log(">> secret created inside function :: ",{sec})
      ...
      console.log(">>> maching :: ", {token, secret})
      return token
    }
    ...
    if (!secret) {
      ...
      console.log(">> secret created outside function :: ",{secret})
      setSecret(req, res, sessionKey, secret, cookie)
    }

    // verify the incoming token
    console.log(">> checking area :: ", {reqM: req.method, fc: ignoreMethod[req.method], secret, value: value(req), verify: tokens.verify(secret, value(req))  })
